...
function normalize(input: any) {
  ...
  if (b && typeof b === "object" && !Array.isArray(b)) {
    ...
    if (
      keys.length > 0 &&
      keys.every((k) => /^\d+$/.test(k) && typeof (b as any)[k] === "string")
    ) {
      try {
        const s = keys
          .map((k) => parseInt(k, 10))
...
async function getPdvTable(): Promise<string> {
  ...
  if (!PDV_TABLE)
    throw new Error(
      "No PDV table found: expected point_de_vente or points_de_vente",
    );
  return PDV_TABLE;
}
...
export const createPointDeVente: RequestHandler = async (req, res) => {
  try {
    ...
    if (!evenement_id || !nom) {
      return res
        .status(400)
        .json({ error: "Missing required fields: evenement_id, nom" });
    }
